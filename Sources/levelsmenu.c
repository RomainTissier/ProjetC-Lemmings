...
LevelsMenu *levelsmenu_create(SDL_Renderer *render) {
	...
	for (i = 0; i < NBLEVELS; i++)
		SDL_RenderCopy(render, menu->levelsButtons[i]->background, NULL,
				&(menu->levelsButtons[i]->position));
	SDL_RenderCopy(render, menu->returnButton->background, NULL,
			&(menu->returnButton->position));
...
int levelsmenu_execute(LevelsMenu *menu) {
	...
	while (SDL_WaitEvent(&event)) {
		...
		case SDL_MOUSEBUTTONUP:
			...
			if (collisionDetectionCursorRect(x, y, menu->returnButton->position)
					== POINT)
				return -1;
			for (i = 0; i < NBLEVELS; i++) {
				if (collisionDetectionCursorRect(x, y,
						menu->levelsButtons[i]->position) == POINT)
